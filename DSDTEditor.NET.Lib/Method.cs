...
namespace DSDTEditor.NET.Lib
{
    public class Method : DSDTProperty, IACPIObject
    {
        ...
        {
            this.Name = fileContent[currentPosition].Trim().Replace("Method (", string.Empty).Replace(")", string.Empty);
            currentPosition++;
            bool table = false;
            int level = 0;
            while (currentPosition < fileContent.Count)
            {
                ...
                if (trimmedString.StartsWith("{"))
                {
                    if (!trimmedString.Contains("}"))
                    {
                        ...
                        {
                            level++;
                            Debug.WriteLine("[Method] Line: {0} - Level: {1} - Inhalt: {2}", currentPosition + 1, level, fileContent[currentPosition]);
                        }
                        else
                        {
                            table = true;
